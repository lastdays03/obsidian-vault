...
#### 3.3.2 기술적 메커니즘 (Official Standard)
Claude Code의 공식 스킬 정의 표준은 **디렉토리 기반(Directory-based)**입니다. 각 스킬은 `~/.claude/skills/` 하위에 고유한 폴더를 가집니다.

- **필수 파일**: `SKILL.md` (메타데이터 및 절차 정의)
- **선택 폴더**: `scripts/` (실행 스크립트), `resources/` (참조 데이터)

**스킬 정의 예시 (`sql-query/SKILL.md`):**
```markdown
name: query-database
...
#### 3.3.5 대규모 운용 시 주의사항 (Limits & Best Practices)
스킬이 너무 많아지면(50개 이상) 다음과 같은 문제가 발생할 수 있습니다. 메타데이터(Description)는 항상 시스템 프롬프트에 상주하기 때문입니다.

1.  **컨텍스트 과부하(Context Overload)**: 설명이 길거나 많으면 메타데이터만으로 수천 토큰을 소모합니다.
2.  **인식 저하(Unreliable Activation)**: 비슷한 스킬이 충돌하여 활성화율이 20% 미만으로 떨어질 수 있습니다.
3.  **관리 복잡성**: 100개 이상의 파일은 YAML 문법 오류 등의 유지보수 비용을 급증시킵니다.

> **💡 Best Practices**
> - **수량 제한**: 스킬은 **10~30개** 사이가 가장 이상적이며, 최대 50개를 넘기지 마십시오.
> - **설명 최적화**: Description은 1,024자 이내로 짧고 구체적("Use when...")으로 작성하십시오.
> - **하이브리드 전략**: CLAUDE.md에 주요 스킬 요약 + 포인터(파일 경로) 추가 → 자동 활성화 강화.
...
### 3.6 플러그인 시스템 (Plugin System): 커스터마이징의 정점

#### 3.6.1 핵심 개념 (Core Concept)
2025년 10월 도입된 플러그인 시스템은 슬래시 커맨드, 서브에이전트, MCP 서버, 훅(Hooks) 등을 하나의 패키지로 묶어 배포하고 설치할 수 있게 해주는 기능입니다. 이를 통해 Claude Code를 단순한 도구에서 "팀 맞춤형 개발 플랫폼"으로 진화시킬 수 있습니다.

#### 3.6.2 주요 구성 요소
- **Slash Commands**: `/review`와 같은 단축 명령어.
- **Subagents**: 코드 리뷰어, 디버거 등 특수 목적 에이전트.
- **MCP Servers**: 외부 도구 연결 (DB, 브라우저 등).
- **Hooks**: 세션 시작/종료 등 특정 시점에 자동 실행되는 로직 (예: 시작 시 `git pull` 자동 수행).

#### 3.6.3 마켓플레이스 및 설치 (Marketplace & Installation)
플러그인은 "마켓플레이스"를 등록한 후 설치하는 구조입니다.

**1. 마켓플레이스 추가 (Add Marketplace):**
플러그인 카탈로그를 등록합니다.
```bash
> /plugin marketplace add [소스]
```
- **공식 (Official)**: `anthropics/claude-code` (기본 탑재)
- **Superpowers**: `obra/superpowers-marketplace` (파워 유저 필수)
- **Repomix**: `yamadashy/repomix` (코드베이스 분석)
- **Claude Night Market**: `athola/claude-night-market` (다양한 커뮤니티 워크플로우)

**2. 플러그인 설치 (Install Plugin):**
- **인터랙티브 (초보자 추천)**: `/plugin` 입력 후 "Browse and install" 선택.
- **직접 설치 (숙련자)**: `/plugin install [플러그인명]@[마켓플레이스명]`
  - 예: `/plugin install superpowers@superpowers-marketplace`

#### 3.6.4 플러그인 관리 및 주의사항
- **목록**: `/plugin list`
- **관리**: `/plugin update`, `/plugin remove`
- **제어**: `/plugin enable` / `/plugin disable` (사용하지 않는 플러그인은 비활성화하여 컨텍스트 절약)

> [!CAUTION] Security & Performance
> - **보안**: 플러그인은 MCP 서버를 포함하여 민감한 데이터에 접근할 수 있습니다. **반드시 신뢰할 수 있는 소스만 추가하십시오.**
> - **성능**: 플러그인도 스킬처럼 메타데이터를 로드합니다. 설치는 신중하게 하고, 20~50개 제한을 지키십시오. 

---

#### 3.6.5 Pro Tip: 필수 추천 플러그인 (Essential Recommendations)
> [!TIP] Expert Insight
> **Superpowers (obra/superpowers)**는 반드시 설치해야 할 "확장팩 1순위"입니다. TDD 강제, 정교한 디버깅 루프 등 파워 유저를 위한 핵심 기능들이 집약되어 있습니다.
>
> 더 많은 추천 플러그인(Top 20)과 상세 설명은 별도의 **[[Claude_Plugin_Registry|Claude Code Plugin Registry (2026 Edition)]]** 문서를 참조하십시오.
